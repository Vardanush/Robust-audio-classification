...
# uncomment above to get pretrained model
...
class ResNet(nn.Module):
    ...
    def __init__(
            ...
            norm_layer: Optional[Callable[..., nn.Module]] = None, num_input_channels=10
    ) -> None:
        ...
        self.conv1 = nn.Conv1d(num_input_channels, self.inplanes, kernel_size=80, stride=4, padding=39,
                               bias=False)
        self.bn1 = norm_layer(self.inplanes)
        self.relu = nn.ReLU(inplace=True)
        self.maxpool = nn.MaxPool1d(kernel_size=4, stride=4, padding=1)
        self.layer1 = self._make_layer(block, 48, layers[0])
        self.layer2 = self._make_layer(block, 96, layers[1], stride=1,
                                       dilate=replace_stride_with_dilation[0])
        self.layer3 = self._make_layer(block, 192, layers[2], stride=1,
                                       dilate=replace_stride_with_dilation[1])
        self.layer4 = self._make_layer(block, 384, layers[3], stride=1,
